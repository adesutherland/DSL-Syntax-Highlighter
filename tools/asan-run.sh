...
DEFAULT_FOCUSED_REGEX='^(parser_tester_reinit|ep_test|stdio_test|stdio_shutdown_test)$'

build_dir="cmake-build-asan"
build_jobs=4
test_jobs=4
test_jobs_set=0
build_targets=()
...
Phases:
  --phase build          CMake build only.
  --phase full           Build, then run all CTests. Default test leaks: on.
  --phase ctest          Run CTest only. Use after a build.
  --phase focused-lsan   Build focused targets, then focused LSan tests.
  --phase rerun-failed   Run CTest --rerun-failed.

Options:
  --build-dir DIR        Build directory. Default: cmake-build-asan.
...
Examples:
  ...
  tools/asan-run.sh --phase build --build-target parser_tester --build-target tp
USAGE
}
...
case "$phase" in
    ...
    ctest)
        ...
        ;;
    focused-lsan)
        run_cmake_build focused-build --target parser_tester tp ep_test stdio_test stdio_shutdown_test
