...
static void test_c_parser_tree_and_tokens(void) {
    const char *source =
        "#include <stdio.h>\n"
        "int main(void) {\n"
        "  // hello\n"
    ...
    InitialLoad *load = create_initial_load("test.c", source);
    ...
    assert(count_nodes_of_type(cb->parse_tree->root, LEXER_PREPROCESSOR) >= 1);
    ...
    assert(first != NULL);
    assert(first->type == LEXER_PREPROCESSOR);
...
int main(void) {
    ...
    test_c_parser_tree_and_tokens();
    printf("C parser tests passed!\n");
    return 0;
}
