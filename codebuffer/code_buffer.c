...
/* Common Helper Function to set the Cd_Buffer with an initial load.
 * It frees the initial load after setting the code buffer.
 */
static void set_codebuffer_with_initial_content(CodeBuffer *cb, InitialLoad *initial_load) {
    if (!cb) return;
...
void load_initial_content(CodeBuffer *cb, InitialLoad *initial_load) {
     ...
     * - This also frees initial_load
     */
    set_codebuffer_with_initial_content(cb, initial_load);
    ...
    if (cb->parser_function) {
        ...
        /* We need to create the initial emergency parse tree, etc. */
    ...
    highlight_syntax(cb); // Highlight the syntax of the editor CodeBuffer - emergency parser
