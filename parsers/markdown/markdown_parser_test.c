...
static void test_markdown_parser_tree_and_tokens(void) {
    const char *source =
        "# Title\n"
        "\n"
        "- item `code`\n"
    ...
    InitialLoad *load = create_initial_load("test.md", source);
    ...
    assert(count_nodes_of_type(cb->parse_tree->root, PARSE_TREE_STRUCTURE) >= 1);
    assert(count_nodes_of_type(cb->parse_tree->root, PARSE_TREE_SCOPE) >= 1);
    assert(count_nodes_of_type(cb->parse_tree->root, PARSE_TREE_CODEBLOCK) >= 1);
    assert(count_nodes_of_type(cb->parse_tree->root, LEXER_PREPROCESSOR) >= 2);
    assert(count_nodes_of_type(cb->parse_tree->root, LEXER_STRING_LITERAL) >= 2);

    CB_Node *heading_marker = find_leaf_at(cb->parse_tree->root, 0);
    assert(heading_marker != NULL);
    assert(heading_marker->type == LEXER_PREPROCESSOR);

    free_code_buffer(cb);
}

int main(void) {
    printf("Testing Markdown parser...\n");
    test_markdown_parser_tree_and_tokens();
    printf("Markdown parser tests passed!\n");
    return 0;
}
