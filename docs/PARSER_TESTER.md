...
3. **Timeouts are Critical**: Originally, the test harness would block forever if the parser silently died without closing `STDOUT` or if it went into an infinite loop. We added a 10-second timeout to `wait_for_parser()`. If the parser fails to return an AST within 10 seconds, the test automatically fails, preventing CI/CD lockups.
