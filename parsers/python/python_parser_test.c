...
static void test_python_parser_tree_and_tokens(void) {
    const char *source =
        "import math\n"
        "def square(x):\n"
        "    # hello\n"
    ...
    InitialLoad *load = create_initial_load("test.py", source);
    ...
    assert(count_nodes_of_type(cb->parse_tree->root, PARSE_TREE_STATEMENT) >= 1);
...
int main(void) {
    printf("Testing Python parser...\n");
    test_python_parser_tree_and_tokens();
    printf("Python parser tests passed!\n");
    return 0;
}
